document.addEventListener('DOMContentLoaded',()=>{
    ...
    function createBoard(){
        ...
        for(let i = 0 ; i < sqaures.length;i++){
            ...
            const isRightEdage = (i % width === width-1)

            if(sqaures[i].classList.contains('valid')){
                if(i > 0 && !isLeftEdge && sqaures[i-1].classList.contains('bomb')) total++
                if(i > 9 && !isRightEdage && sqaures[i+1-width].classList.contains('bomb')) total++
                if(i > 10 && !sqaures[i-width].classList.contains('bommb')) total++
                if(i > 11 && !isLeftEdge && sqaures[i-1-width].classList.contains('bomb')) total++
